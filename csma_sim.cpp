...
int main(int argc, char* argv[]) {
    ...
    // ====================================================
    // 1. РЕАЛИЗАЦИЯ ГЕНЕРАЦИИ ПАКЕТОВ
    // ====================================================
    // Используем экспоненциальное распределение для интервалов между пакетами
    // Экспоненциальное распределение Exp(α) соответствует пуассоновскому потоку
    ...
    // ====================================================
    // 2. РЕАЛИЗАЦИЯ БУФЕРА АДАПТЕРА (ОЧЕРЕДИ ПАКЕТОВ)
    // ====================================================
    // Каждый хост имеет свою собственную очередь (deque)
    // Это соответствует архитектуре реальных сетевых адаптеров
    vector< deque<Packet> > queues(N);
    
    // Статистика по очередям для вычисления метрик
    vector<double> last_queue_change_time(N, 0.0);    // время последнего изменения очереди
    vector<double> queue_time_area(N, 0.0);           // интеграл длины очереди по времени
    vector<int> max_queue_len(N, 0);                  // максимальная длина очереди для каждого хоста
    // ====================================================
    ...
    // ====================================================
    // 3. МОДЕЛИРОВАНИЕ ПРОЦЕССА ПЕРЕДАЧИ (CSMA ЛОГИКА)
    // ====================================================
    // Состояние канала CSMA
    bool channel_busy = false;               // флаг занятости канала
    double channel_busy_start = 0.0;         // время начала текущей передачи
    double cumulative_channel_busy = 0.0;    // суммарное время занятости канала
    int current_tx_host = -1;                // хост, который сейчас передает
    uint64_t current_tx_pktid = 0;           // ID передаваемого пакета
    int current_tx_attempts = 0;             // число попыток для текущей передачи
    // ====================================================
    ...
    while (!eq.empty()) {
        ...
        if (ev.type == EVT_ARRIVAL) {
            ...
            // ====================================================
            // CSMA ЛОГИКА: Проверка условий для немедленной передачи
            // ====================================================
            ...
            if (!channel_busy && !somebody_waiting && queues[h].empty()) {
                ...
                // ====================================================
                // РАБОТА С БУФЕРОМ АДАПТЕРА: Добавление в очередь или отбрасывание
                // ====================================================
            ...
            // ====================================================
            // ГЕНЕРАЦИЯ ПАКЕТОВ: Планирование следующего прибытия
            // ====================================================
        ...
        // === СОБЫТИЕ: КОНЕЦ ПЕРЕДАЧИ ===
            ...
            // ====================================================
            // CSMA ЛОГИКА: Обновление статистики занятости канала
            // ====================================================
            if (channel_busy) {
            ...
            // ====================================================
            // CSMA ЛОГИКА: Выбор следующего пакета для передачи
            // ====================================================
